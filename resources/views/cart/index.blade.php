                                ...
                                <th scope="row" class="border-0">
                                    <div class="p-2">
                                        ...
                                        <div class="ml-3 d-inline-block align-middle">
                                            <h5 class="mb-0"> <a href="{{ route('template.about', ['nom' => $product->model->nom]) }}" class="text-dark d-inline-block align-middle">{{ $product->model->nom }}</a></h5><span class="text-muted font-weight-normal font-italic d-block">Category:</span>
                                        </div>
                                    </div>
                                </th>
                                <td class="border-0 align-middle"><strong>{{ $product->model->getPrice() }}</strong></td>
                                <td class="border-0 align-middle"><strong style="margin-left:50px;">1</strong></td>
                                <td class="border-0 align-middle">
                                    <form action="{{ route('cart.destroy', $product->rowId) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger" style="margin-left:30px;"><i class="fa fa-trash"></i></a>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
...
<div class="col-md-12">
    ...
    <p>Mais vous pouvez visiter la <a href="{{ route('template.index') }}">boutique</a> pour faire votre shopping.
    </p>
</div>
@endif
@endsection
